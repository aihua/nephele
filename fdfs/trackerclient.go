...
func (this *trackerClient) queryStroageStoreWithGroup(groupName string) (*storageInfo, error) {
	...
	//package length
	binary.Write(buffer, binary.BigEndian, int64(FDFS_GROUP_NAME_MAX_LEN))
	//cmd
	buffer.WriteByte(byte(TRACKER_PROTO_CMD_SERVICE_QUERY_STORE_WITH_GROUP_ONE))
	//status
	buffer.WriteByte(byte(0))
	//16 bit groupName
	buffer.WriteString(fixString(groupName, 16))
